...
let stress =
  DEV_MODE
    ? 0
    : (Number(localStorage.getItem("stress")) || 0);


/*
  MONEYはDEV_MODEでもlocalStorageを使用。
  NEO TOKYOとTHE SEVENTHで共通残高になる。
*/
...
const identityLines = {
  ...
  "GUEST-01":
    "仮アクセスIDだね。制限付きで接続できるよ。",

  "CITIZEN-205":
    "市民IDを確認。低階層アクセスを許可するね。",

  "UPPER-771":
    "上層IDを確認。少し広い範囲まで見せられるよ。",
  ...
  return (
    ...
    voices.find(
      v =>
        ...
        /kyoko|haruka|female|woman|siri/i.test(v.name)
    )
...
function clearTypeTimer(name) {
  ...
  if (
    ...
    clearInterval(typeTimerDistrict);
  }
...
function updateHUD() {
  ...
  rankDisplay2.textContent =
    rankText;

  stressDisplay2.textContent =
    stressText;

  moneyDisplay2.textContent =
    moneyText;
  ...
  if (cyberScanbar) {
    ...
    if (stress >= 50) {
      ...
      cyberScanbar.classList.add(
        "scanbar-on"
      );

    } else {

      cyberScanbar.classList.remove(
        "scanbar-on"
      );
...
function moveWatchReticle(
  ...
  watchReticle.style.left =
    `${x}px`;

  watchReticle.style.top =
    `${y}px`;

  if (active) {

    watchReticle.classList.add(
      "active"
    );
...
function forceLogoutSequence() {
  ...
  const logoutMessage =
    "ストレス値が上限に達しました。安全のため接続を遮断します。";


  if (currentDistrict === "slum") {

    slumGlitchText.textContent =
      "STRESS LIMIT / CONNECTION TERMINATED";

    slumGlitch.classList.remove(
      "hidden"
    );
  ...
  if (
    districtScreen.classList.contains(
      "active"
    )
  ) {
...
const districtData = {
  ...
  1: {
    ...
    sub:
      "CORE DISTRICT / CENTRAL NEXUS",

    desc:
      "NEO AI中枢領域です。制限区域に指定されています。監視密度は都市内で最上位です。",
  ...
  2: {
    ...
    sub:
      "UPPER DISTRICT / HIGH NETWORK",

    desc:
      "上層ネットワーク区域です。高層空間通路と上位生活圏で構成されています。",
  ...
  3: {
    ...
    sub:
      "GATE DISTRICT / TRANSIT LINK",

    desc:
      "交通および出入口管区です。都市の移動経路を統括しています。",
  ...
  4: {
    ...
    sub:
      "COMMERCE DISTRICT / PURCHASE LOG",

    desc:
      "商業管区です。消費行動は監視対象です。IDスキャン決済が行われます。",
  ...
  5: {
    ...
    sub:
      "ENTERTAINMENT DISTRICT / CYBER ARENA",

    desc:
      "観光およびサイバーアリーナ区域です。娯楽プロトコルが展開されています。",
  ...
  6: {
    ...
    sub:
      "LIFE DISTRICT / LOW CONTROL",

    desc:
      "下町系低管理区域です。生活供給ノードが稼働中です。",
  ...
  7: {
    ...
    sub:
      "CASINO RESORT / THE SEVENTH",

    desc:
      "NEO TOKYO最大級の統合型リゾート区域です。カジノ、ホテル、商業施設、エンターテインメント施設が24時間稼働しています。",
...
function accessDistrict(num) {
  ...
  */
  ...
  districtDesc.textContent =
    data.desc;

  districtSubLabel.textContent =
    data.sub;
  ...
  if (num === 4) {
    ...
    setTimeout(() => {
      ...
      paymentText.textContent =
        "IDスキャン決済を実行しますか？ / 500 CREDIT";


      const openPayMessage =
        pick(
          aiLines.payOpen
        );
...
payYes.addEventListener(
  ...
  () => {
    ...
    setSideAI(
      `${selectedIdentity.id} のIDを見てる。少しだけ待って。`
    );
...
if (enterCasinoBtn) {
  ...
  enterCasinoBtn.addEventListener(
    "click",
    () => {
...
/*
  casino.htmlから戻ったときなど、
  ページが再表示されたタイミングで
  最新残高をlocalStorageから読み直す。
*/
